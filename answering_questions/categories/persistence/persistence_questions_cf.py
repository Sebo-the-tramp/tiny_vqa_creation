...
# def CF_PERSISTENCE_OBJECT_PRESENT(
...
# this is also okay
# @with_resolved_attributes_cf
# def CF_PERSISTENCE_OBJECT_DISAPPEAR(
#    world_state_og: WorldState, world_state_mod: WorldState, question: QuestionPayload, attributes, **kwargs
# ) -> Sequence[str]:
    
#     return CF_PERSISTENCE_OBJECT_PRESENT(
#         world_state_og, world_state_mod, question, attributes, **kwargs
#     )
...
def CF_PERSISTENCE_OBJECT_TOTAL_COUNT(
   world_state_og: WorldState, world_state_mod: WorldState, question: QuestionPayload, attributes, **kwargs
    ...
    visibility_mask, _, _ = get_visibility_mask(world_state_mod)
    total_visible_objects = np.sum(visibility_mask, axis=0)

    initial_timestep_index, initial_timestep, final_timestep_index, final_timestep = get_optimal_timestep_interval(world_state_mod)
    ...
    final_timestep = list(world_state_mod["simulation"].keys())[final_timestep_index]
    initial_timestep = list(world_state_mod["simulation"].keys())[initial_timestep_index]
    ...
    resolved_attributes = resolve_attributes_visible_at_timestep(
        attributes, world_state_mod, final_timestep
    )

    return fill_questions_cf(
        question, labels, correct_idx, world_state_og, world_state_mod, final_timestep, resolved_attributes, initial_timestep=initial_timestep
    )
...
# @with_resolved_attributes_cf
# def CF_PERSISTENCE_OBJECT_COLLISION_HIDDEN(
#    world_state_og: WorldState, world_state_mod: WorldState, question: QuestionPayload, attributes, **kwargs
# ) -> Sequence[str]:
#
#     assert len(attributes) == 0
