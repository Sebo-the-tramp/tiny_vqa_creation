...
def _normalize_latex_table(latex_text: str, size_cmd: str = r"\tiny") -> str:
    # Ensure tables are compact; fall back to inserting size before tabular.
    if r"\small\begin{tabular}" in latex_text:
        latex_text = latex_text.replace(
            r"\small\begin{tabular}", f"{size_cmd}\\begin{{tabular}}"
        )
    else:
        latex_text = latex_text.replace(
            r"\begin{tabular}", f"{size_cmd}\\begin{{tabular}}"
        )
    return latex_text


def _print_one_line(label: str, latex_text: str) -> None:
    one_line = " ".join(latex_text.splitlines())
...
def main() -> None:
    ...
    single_path = output_dir / "heatmap_table_single.tex"
    multi_path = output_dir / "heatmap_table_multi.tex"
    latex_single = print_heatmap_table_latex(acc_mat_single, output_path=str(single_path))
    latex_multi = print_heatmap_table_latex(acc_mat_multi, output_path=str(multi_path))
    latex_single = _normalize_latex_table(latex_single)
    latex_multi = _normalize_latex_table(latex_multi)
    single_path.write_text(latex_single, encoding="utf-8")
    multi_path.write_text(latex_multi, encoding="utf-8")
    _print_one_line("heatmap_table_single", latex_single)
    _print_one_line("heatmap_table_multi", latex_multi)
