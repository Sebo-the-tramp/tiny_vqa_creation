...
def _load_impl_module(mock: bool):
    modname = ".meta_mock" if mock else ".meta_real"
    return import_module(modname, package=__package__)


def get_function_by_name_meta(name: str, mock: bool = False) -> Resolver:
...
def _load_gt_module(mock: bool):
    modname = ".meta_mock_results"
    return import_module(modname, package=__package__)


def get_result_by_name_meta(name: str, mock: bool = False) -> Any:
