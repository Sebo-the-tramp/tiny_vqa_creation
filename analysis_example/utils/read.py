...
pattern = re.compile(
    r"\b(" + "|".join(map(re.escape, list_of_objects)) + r")\b", re.IGNORECASE
)
...
def load_from_model_records(model_records: list[dict]):
    ...
    if "params_b" in models_df.columns:
        models_df["params_b"] = (
            models_df["params_b"].map(_parse_params_b).astype("Float32")
        )

    if "release_year" in models_df.columns:
        models_df["release_year"] = pd.to_numeric(
            models_df["release_year"], errors="coerce"
        ).astype("Int16")
    ...
    models_df = models_df[
        [c for c in keep_cols if c in models_df.columns]
        ...
        for r in m["results"]:
            ...
            if idx not in item_rows:
                ...
                if prev is not None and cur is not None and prev != cur:
                    raise ValueError(
                        f"Inconsistent ground-truth for idx={idx}: {prev} vs {cur}"
                    )
    ...
    if "params_b" in eval_df.columns:
        eval_df["params_b"] = pd.to_numeric(
            eval_df["params_b"], errors="coerce"
        ).astype("Float32")
    if "release_year" in eval_df.columns:
        eval_df["release_year"] = pd.to_numeric(
            eval_df["release_year"], errors="coerce"
        ).astype("Int16")
            ...
            if simulation_path in simulations_metadata_cache:
                ...
                if "scratch" in sim_path:
                    sim_path = sim_path.replace(
                        "/scratch/project/eu-25-92/composite_physics/dataset/simulation_v4",
                        "/data0/sebastian.cavada/datasets/simulations_v4",
                    )
                ...
                if os.path.exists(sim_path):
                    ...
                    raise FileNotFoundError(
                        f"Simulation metadata file not found: {sim_path}"
                    )
