...
def print_summary_models_used(eval_df):
    ...
    eval_df_single_image_models_unique = eval_df[eval_df['idx'].str.contains('_i')]['model_id'].unique()
    eval_df_multi_image_models_unique = eval_df[eval_df['idx'].str.contains('_g')]['model_id'].unique()

    single_images = np.setdiff1d(eval_df_single_image_models_unique, eval_df_multi_image_models_unique)
    multi_images = eval_df_multi_image_models_unique

    # 1. Load Metadata
    try:
        with open('/data0/sebastian.cavada/compositional-physics/tiny_vqa_deterministic/analysis_example/utils/metadata.json', 'r') as f:
    ...
    latex_output.append(r"This subsection provides specifications for multi-image and general models. Those models can handle multiple images as input. In our setup we provide up to eight images sampled uniformly.")
...
def print_heatmap_table_latex(acc_mat: pd.DataFrame, output_path: str = "heatmap_table.txt") -> str:
    ...
    def latex_tables_by_vlm_chunks(df, chunk_size=10, cmap="RdYlGn"):
        ...
        for start in range(0, n_cols, chunk_size):
            ...
            inner_latex = inner_latex.replace(r"\begin{tabular}", r"\small\begin{tabular}")
            inner_latex = inner_latex.replace(r"\textbf{Total}", r"\midrule \textbf{Total}")
