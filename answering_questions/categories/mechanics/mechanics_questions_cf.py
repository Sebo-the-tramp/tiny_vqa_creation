...
def CF_COLLISIONS_OBJ_OBJ_FIRST(
    ...
    **kwargs,
    ...
    timestep_index_start = int(
        answer_list_original_data_cf[0][3][0]
    )  # this has to be the image to get the question
    timestep_start = get_timestep_from_idx(timestep_index_start)

    object_id = answer_list_original_data_cf[0][5]["OBJECT"]["choice"]["id"]
    usable_mask = get_mask_collisions(world_state_og)[
        timestep_index_start : timestep_end_index + 1, int(object_id), 1:
    ]  # only object-object collisions

    rows = usable_mask.any(axis=1)
    t_first = np.argmax(rows) if rows.any() else None

    if t_first is not None:
        obj_first = int(np.argmax(usable_mask[t_first]))
    else:
        obj_first = None
    ...
    if obj_first is not None and obj_first > 0:
        labels, correct_idx = create_mc_object_names_from_dataset(
            world_state_og["objects"][str(obj_first)]["name"], present, DATASET
        )
    else:
        labels, correct_idx = create_mc_object_names_from_dataset(
