...
for experiment in sorted(os.listdir(path)):
    ...
    with open(
        os.path.join(experiment_path, f"test_{experiment}_10K_copy.json"),
        "r",
        encoding="utf-8",
    ) as f:
        data = json.load(f)
        ...
        for question in data:
            ...
            wrong_indexes = [
                i for i, file in enumerate(file_names) if "//render" in file
            ]
            count_wrong = len(wrong_indexes)
            file_names = [
                file for i, file in enumerate(file_names) if i not in wrong_indexes
            ]
            question["file_name"] = file_names

            count_images = len(file_names)
            question["question"] = question["question"].replace(
                "<image>" * count_images, "<image>" * (count_images - count_wrong)
            )
    ...
    with open(
        os.path.join(experiment_path, f"test_{experiment}_10K.json"),
        "w",
        encoding="utf-8",
    ) as f:
