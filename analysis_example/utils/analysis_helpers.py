...
def summarize_accuracy(
    df, by=("model_id",), sort=("accuracy", "n"), ascending=(False, False)
):
    g = (
        df.groupby(list(by), observed=True, dropna=False)
        .agg(n=("idx", "count"), correct=("is_correct", "sum"))
        .reset_index()
    )
...
def confusion(df, model_id):
    ...
    return (
        pd.crosstab(sub["correct_answer"], sub["answer"], dropna=False)
        .fillna(0)
        .astype(int)
    )
...
def error_table(df, model_id, k=50):
    sub = df[
        (df["model_id"] == model_id) & (df["split"] == "val") & (~df["is_correct"])
    ]
    cols = [
        "idx",
        "scene",
