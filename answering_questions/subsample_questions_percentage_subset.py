...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional JSON file containing the superset of questions eligible for sampling. "
            "Only entries present in this file (matched by idx) will be considered."
        ),
    )
    parser.add_argument(
        "--percentage-map",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Preserve the split of the 'mode' field (e.g., general vs image-only) observed after "
            "filtering/subsetting. Sampling will fail if ratios cannot be satisfied because of the "
            "percentage map or insufficient data."
        ),
    )
    parser.add_argument(
        "--seed",
...
def compute_mode_counts(records: Sequence[dict[str, Any]], mode_field: str) -> Dict[Any, int]:
    counts: DefaultDict[Any, int] = defaultdict(int)
    for record in records:
        if mode_field not in record:
            raise SystemExit(
                f"Cannot preserve mode ratios because field '{mode_field}' is missing from a record."
            )
        counts[record[mode_field]] += 1
    return dict(counts)


def allocate_mode_targets(total_required: int, mode_counts: Mapping[Any, int]) -> Dict[Any, int]:
    if total_required <= 0:
        return {mode: 0 for mode in mode_counts}

    total_available = sum(mode_counts.values())
    if total_available <= 0:
        raise SystemExit("Cannot preserve mode ratios because no data is available after filtering.")

    total_required = min(total_required, total_available)
    desired: Dict[Any, float] = {}
    allocations: Dict[Any, int] = {}
    assigned = 0
    for mode, count in mode_counts.items():
        share = (count / total_available) * total_required
        desired[mode] = share
        allocation = min(int(math.floor(share)), count)
        allocations[mode] = allocation
        assigned += allocation

    remainder = total_required - assigned
    if remainder < 0:
        raise AssertionError("Mode allocation over-assigned before distributing remainder.")

    while remainder:
        candidates = [
            mode
            for mode in desired
            if allocations[mode] < mode_counts[mode]
        ]
        if not candidates:
            break
        candidates.sort(
            key=lambda mode: (desired[mode] - allocations[mode], mode_counts[mode] - allocations[mode]),
            reverse=True,
        )
        chosen = candidates[0]
    ...
    if remainder:
        raise SystemExit(
            "Unable to preserve mode ratios because the requested distribution exceeds available data."
        )
    return allocations


class _FlowEdge:
    __slots__ = ("to", "rev", "cap", "initial_cap")
...
class _Dinic:
    ...
    def add_edge(self, u: int, v: int, cap: int) -> _FlowEdge:
        forward = _FlowEdge(v, len(self.graph[v]), cap)
        backward = _FlowEdge(u, len(self.graph[u]), 0)
        self.graph[u].append(forward)
        self.graph[v].append(backward)
        return forward

    def _bfs(self, source: int, sink: int) -> List[int] | None:
        level = [-1] * self.size
        level[source] = 0
    ...
    def _dfs(self, node: int, sink: int, flow: int, level: List[int], iter_idx: List[int]) -> int:
        if node == sink:
            return flow
        for i in range(iter_idx[node], len(self.graph[node])):
            iter_idx[node] = i
            edge = self.graph[node][i]
            if edge.cap <= 0 or level[node] >= level[edge.to]:
                continue
            pushed = self._dfs(edge.to, sink, min(flow, edge.cap), level, iter_idx)
            if pushed > 0:
                edge.cap -= pushed
    ...
    def max_flow(self, source: int, sink: int) -> int:
        ...
        while True:
            ...
            if level is None:
                break
            iter_idx = [0] * self.size
            while True:
                pushed = self._dfs(source, sink, math.inf, level, iter_idx)
                if pushed == 0:
                    break
                flow += pushed
        return flow
...
def assign_group_mode_counts(
    grouped_by_mode: Dict[Tuple[Any, ...], Dict[Any, List[dict[str, Any]]]],
    takes: Mapping[Tuple[Any, ...], int],
    mode_targets: Mapping[Any, int],
) -> Dict[Tuple[Any, ...], Dict[Any, int]]:
    active_groups = [key for key, count in takes.items() if count > 0]
    total_required = sum(takes[key] for key in active_groups)
    if total_required == 0:
        return {}

    filtered_mode_targets = {mode: count for mode, count in mode_targets.items() if count > 0}
    if not filtered_mode_targets:
        return {}

    mode_list = list(filtered_mode_targets.keys())
    mode_index = {mode: idx for idx, mode in enumerate(mode_list)}
    group_index = {key: idx for idx, key in enumerate(active_groups)}

    source = 0
    mode_offset = 1
    group_offset = mode_offset + len(mode_list)
    sink = group_offset + len(active_groups)

    dinic = _Dinic(sink + 1)
    for mode, target in filtered_mode_targets.items():
        node = mode_offset + mode_index[mode]
        dinic.add_edge(source, node, target)

    edge_lookup: Dict[Tuple[Any, Tuple[Any, ...]], _FlowEdge] = {}
    for key in active_groups:
        group_node = group_offset + group_index[key]
        dinic.add_edge(group_node, sink, takes[key])
        per_mode = grouped_by_mode.get(key, {})
        for mode, node_idx in mode_index.items():
            capacity = len(per_mode.get(mode, []))
            if capacity <= 0:
                continue
            edge = dinic.add_edge(mode_offset + node_idx, group_node, capacity)
            edge_lookup[(mode, key)] = edge

    flow = dinic.max_flow(source, sink)
    if flow != total_required:
        supplied: DefaultDict[Any, int] = defaultdict(int)
        for (mode, key), edge in edge_lookup.items():
            supplied[mode] += edge.initial_cap - edge.cap

        shortfalls = [
            f"{mode}: need {filtered_mode_targets[mode]} but only {supplied.get(mode, 0)} available"
            for mode in filtered_mode_targets
            if supplied.get(mode, 0) < filtered_mode_targets[mode]
        ]
        details = "; ".join(shortfalls) if shortfalls else "insufficient overlap between modes and balance groups"
        raise SystemExit(
            "Unable to preserve mode ratios under the requested percentage map: " + details + "."
        )

    assignments: Dict[Tuple[Any, ...], Dict[Any, int]] = {}
    for (mode, key), edge in edge_lookup.items():
        used = edge.initial_cap - edge.cap
        if used <= 0:
            continue
        assignments.setdefault(key, {})[mode] = used
    return assignments
...
def allocate_from_percentages(
    ...
    rng: random.Random,
    max_total: int | None,
    mode_counts: Mapping[Any, int] | None = None,
    mode_field: str = "mode",
) -> Tuple[List[dict[str, Any]], Dict[Tuple[Any, ...], float], Dict[Any, int]]:
    ...
    grouped_by_mode: Dict[Tuple[Any, ...], Dict[Any, List[dict[str, Any]]]] = {}
    mode_assignments: Dict[Tuple[Any, ...], Dict[Any, int]] = {}
    mode_targets_result: Dict[Any, int] = {}
    if mode_counts:
        grouped_by_mode = {}
        for key in keys:
            per_mode: DefaultDict[Any, List[dict[str, Any]]] = defaultdict(list)
            for record in grouped[key]:
                mode_value = record.get(mode_field, MISSING_TOKEN)
                per_mode[mode_value].append(record)
            grouped_by_mode[key] = per_mode

        total_required = sum(takes.values())
        mode_targets_result = allocate_mode_targets(total_required, mode_counts)
        mode_assignments = assign_group_mode_counts(grouped_by_mode, takes, mode_targets_result)
    ...
    for key in keys:
        ...
        if take <= 0:
            continue
        if mode_counts:
            assignments = mode_assignments.get(key, {})
            if sum(assignments.values()) != take:
                raise SystemExit(
                    f"Internal error while enforcing mode ratios for group {format_group_key(key)}."
                )
            per_mode_records = grouped_by_mode.get(key, {})
            for mode_value, count in assignments.items():
                if count <= 0:
                    continue
                pool = per_mode_records.get(mode_value, [])
                if len(pool) < count:
                    raise SystemExit(
                        f"Mode '{mode_value}' in group {format_group_key(key)} lacks enough records "
                        f"to satisfy the requested distribution."
                    )
    ...
    rng.shuffle(sampled)
    return sampled, raw_targets, mode_targets_result
...
def main() -> None:
    ...
    if args.subset is not None:
        subset_records = load_questions(args.subset)
        subset_indices: set[str] = set()
        for record in subset_records:
            idx = record.get("idx")
            if idx is None:
                raise SystemExit(
                    f"Subset file '{args.subset}' contains a record without 'idx'; cannot build membership set."
                )
            subset_indices.add(str(idx))
        ...
        for record in questions:
            ...
            if idx is None:
                raise SystemExit(
                    "Encountered a record without 'idx' while enforcing subset membership."
                )
            if str(idx) in subset_indices:
                filtered_questions.append(record)

        if not filtered_questions:
            raise SystemExit(
                f"No overlap between '{args.input}' and subset '{args.subset}' after filtering."
            )
        questions = filtered_questions

    mode_counts: Dict[Any, int] | None = None
    mode_ratios: Dict[Any, float] = {}
    if args.preserve_mode_ratio:
        mode_counts = compute_mode_counts(questions, "mode")
        total_modes = sum(mode_counts.values())
        if total_modes <= 0:
            raise SystemExit("Cannot preserve mode ratios because the filtered dataset is empty.")
        mode_ratios = {mode: count / total_modes for mode, count in mode_counts.items()}
    ...
    sampled, targets, mode_targets = allocate_from_percentages(
        grouped,
        percentages,
        default_percentage,
        rng,
    ...
    if mode_counts:
        final_mode_counts = compute_mode_counts(sampled, "mode")
        final_total = sum(final_mode_counts.values())
        if final_total == 0:
            raise SystemExit("Internal error: sampling produced no records after enforcement.")
        tolerance = max(1.0, 0.01 * final_total)
        mismatches = []
        for mode, ratio in mode_ratios.items():
            expected = ratio * final_total
            actual = final_mode_counts.get(mode, 0)
            if abs(actual - expected) > tolerance:
                mismatches.append((mode, ratio, actual, expected))
        if mismatches:
            details = ", ".join(
                f"{mode}: expected ~{ratio*100:.2f}% ({expected:.1f}) but got {actual}"
                for mode, ratio, actual, expected in mismatches
            )
            raise SystemExit(
                "Unable to preserve mode ratios after enforcing idx grouping or percentage constraints. "
                f"{details}. Consider disabling --group-by-idx or adjusting the subset/percentages."
            )
...
#     --input /data0/sebastian.cavada/compositional-physics/tiny_vqa_deterministic/output/run_08_roi_circling/test_run_08_roi_circling.json \
#     --output /data0/sebastian.cavada/compositional-physics/tiny_vqa_deterministic/output/run_08_roi_circling/test_run_08_roi_circling_1K.json \
