...
def compute_mode_counts(
    records: Sequence[dict[str, Any]], mode_field: str
) -> Dict[Any, int]:
...
def allocate_mode_targets(
    total_required: int, mode_counts: Mapping[Any, int]
) -> Dict[Any, int]:
    if total_required <= 0:
    ...
    if total_available <= 0:
        raise SystemExit(
            "Cannot preserve mode ratios because no data is available after filtering."
        )
    ...
    if remainder < 0:
        raise AssertionError(
            "Mode allocation over-assigned before distributing remainder."
        )

    while remainder:
        candidates = [mode for mode in desired if allocations[mode] < mode_counts[mode]]
        if not candidates:
            break
        candidates.sort(
            key=lambda mode: (
                desired[mode] - allocations[mode],
                mode_counts[mode] - allocations[mode],
            ),
            reverse=True,
        )
        chosen = candidates[0]
...
class _Dinic:
    ...
    def _dfs(
        self, node: int, sink: int, flow: int, level: List[int], iter_idx: List[int]
    ) -> int:
        if node == sink:
            return flow
...
def assign_group_mode_counts(
    ...
    filtered_mode_targets = {
        mode: count for mode, count in mode_targets.items() if count > 0
    }
    ...
    if flow != total_required:
        ...
        details = (
            "; ".join(shortfalls)
            if shortfalls
            else "insufficient overlap between modes and balance groups"
        )
        raise SystemExit(
            "Unable to preserve mode ratios under the requested percentage map: "
            + details
            + "."
        )
...
def allocate_from_percentages(
    ...
    if mode_counts:
        ...
        mode_assignments = assign_group_mode_counts(
            grouped_by_mode, takes, mode_targets_result
        )
...
def main() -> None:
    ...
    if args.preserve_mode_ratio:
        ...
        if total_modes <= 0:
            raise SystemExit(
                "Cannot preserve mode ratios because the filtered dataset is empty."
            )
    ...
    if mode_counts:
        ...
        if final_total == 0:
            raise SystemExit(
                "Internal error: sampling produced no records after enforcement."
            )
