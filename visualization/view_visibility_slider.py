...
def visible_objects_for_timestep(world_state: Mapping, timestep: str) -> List[str]:
    ...
    for obj_id, _ in objects.items():
        if is_object_visible(world_state=world_state, obj_id=obj_id, timestep=timestep):
...
def main() -> None:
    ...
    def update(val: float) -> None:
        ...
        if timestep is None:
            ...
            collision_message = f"t={timestep} | {collision_line}"
            collision_wrapped = textwrap.fill(
                collision_message,
                width=110,
                break_long_words=False,
                break_on_hyphens=False,
            )
            collision_text.set_text(collision_wrapped)
            handles = []
            for obj_id in object_ids:
                ...
                is_visible = (
                    1 if is_object_visible(world_state, obj_id, timestep) else 0
                )
                obj_state = (
                    world_state.get("simulation", {})
                    .get(str(timestep), {})
