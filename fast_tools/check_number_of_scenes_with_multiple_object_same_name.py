...
root_dir = [
    "/data0/sebastian.cavada/datasets/simulations_v3/dl3dv/random",
    "/data0/sebastian.cavada/datasets/simulations_v3/dl3dv/random-cam-stationary",
]
...
for sim_file in list_simulations:
    with open(sim_file, "r") as f:
        ...
        for name, keys in duplicated_names.items():
            if len(keys) > 1:
                print(
                    f"Simulation file {sim_file} has multiple objects with the same name '{name}': {keys}"
                )
