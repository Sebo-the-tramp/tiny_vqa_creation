...
def _load_impl_module(mock: bool):
    modname = ".mass_mock" if mock else ".mass_real"
    return import_module(modname, package=__package__)


def get_function_by_name_mass(name: str, mock: bool = False) -> Resolver:
...
def _load_gt_module(mock: bool):
    modname = ".mass_mock_results"
    return import_module(modname, package=__package__)


def get_result_by_name_mass(name: str, mock: bool = False) -> Any:
