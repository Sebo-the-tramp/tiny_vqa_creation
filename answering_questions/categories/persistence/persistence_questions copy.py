...
def F_PERSISTENCE_OBJECT_TOTAL_COUNT(
    ...
    initial_timestep = list_indexes[initial_timestep_index]

    count_objects_initial = timestep_counts[initial_timestep_index]

    #balanced options around the initial count
    shift = max(0, 2 - count_objects_initial)
    balanced_bins = [str(i) for i in range(count_objects_initial - 2 + shift, count_objects_initial + 2 + shift) if i != count_objects_initial]
...
def F_PERSISTENCE_OBJECT_TOTAL_COUNT_HIDDEN(
    ...
    # I think what is missing is what if an object appears first time in between and disappears before the end?
    # we need to make sure about that.

    count_max = max(timestep_counts[initial_timestep_index:final_timestep_index+1])

    current_answer = count_max - count_objects_final    
    shift = max(0, 2 - current_answer)    
    balanced_bins = [str(i) for i in range(current_answer - 2 + shift, current_answer + 2 + shift) if i != (current_answer)]
