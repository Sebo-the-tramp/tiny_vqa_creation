...
def _load_impl_module(mock: bool):
    modname = ".forces_mock" if mock else ".forces_real"
    return import_module(modname, package=__package__)


def get_function_by_name_forces(name: str, mock: bool = False) -> Resolver:
...
def _load_gt_module(mock: bool):
    modname = ".forces_mock_results"
    return import_module(modname, package=__package__)


def get_result_by_name_forces(name: str, mock: bool = False) -> Any:
